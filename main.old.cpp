...
int lengthOfLongestSubstring(string s)
{
    ...
    for (int i = 1; i <= n; i++)
    {
        //cout << "sign: " << isexist[s[i] - 'a'] << " " << s[i] << " " << i << " " << endl;
        if (isexist[s[i]] != 0 && (isexist[s[i]] - 1) >= sign[0]) //有重复字符 开始位后移
        {
            //cout << "sign[0]_old: " << sign[0];
            sign[0] = isexist[s[i]];
            //cout << " sign[0]_new: " << sign[0] << endl;
        }
        isexist[s[i]] = i + 1;
        sign[1] = i;
        //cout << sign[0] << " " << sign[1] << " " << sign[4] << " " << s[i] << " " << endl;
    ...
    //cout << "sign[2]~sign[3]" << sign[2] << " " << sign[3] << endl;
    return sign[4];
}